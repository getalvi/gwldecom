...
export interface ProcessedImage {
  buffer: Buffer; contentType: "image/webp"; width: number; height: number; bytes: number;
}
...
export async function processProductImage(input: Buffer): Promise<ProcessedImage> {
  ...
  return { buffer: output.data, contentType: "image/webp", width: output.info.width, height: output.info.height, bytes: output.info.size };
}

export function assertValidImageUpload(file: { size: number; type: string }) {
  const MAX_BYTES = 10 * 1024 * 1024;
  const ALLOWED = ["image/jpeg", "image/png", "image/webp", "image/gif", "image/avif"];
  if (!ALLOWED.includes(file.type)) throw new Error(`Unsupported image type: ${file.type}`);
  if (file.size > MAX_BYTES) throw new Error("Image exceeds 10MB upload limit");
}
