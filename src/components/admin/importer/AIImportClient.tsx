  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Import products from Amazon, AliExpress, Daraz, Shopify, WooCommerce, eBay, and any ecommerce site.</p>
      </div>
      <ImportStats/>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex gap-1 overflow-x-auto bg-secondary/30 rounded-xl p-1">
            {TABS.map(({id,label,icon:Icon}) => (
              <button key={id} onClick={()=>setActiveTab(id)} className={`flex items-center gap-1.5 whitespace-nowrap rounded-lg px-3 py-2 text-xs font-medium transition-all ${activeTab===id?"bg-background shadow-sm text-foreground":"text-muted-foreground hover:text-foreground"}`}>
                <Icon size={13}/>{label}
              </button>
            ))}
          </div>
...
function HistoryView() {
  const [jobs, setJobs] = useState<Array<{id:string;type:string;status:string;progress_total:number;progress_done:number;progress_failed:number;created_at:string}>>([]);
  ...
  return (
    <div className="rounded-xl border border-border bg-background overflow-hidden">
      <table className="w-full text-sm">
        <thead className="border-b border-border bg-secondary/30"><tr>{["Type","Total","Done","Failed","Status","Date"].map(h=><th key={h} className="px-3 py-2 text-left text-xs font-semibold text-muted-foreground">{h}</th>)}</tr></thead>
        <tbody className="divide-y divide-border">
          {jobs.map(j=><tr key={j.id} className="hover:bg-secondary/20"><td className="px-3 py-2 text-xs capitalize">{j.type}</td><td className="px-3 py-2 text-xs">{j.progress_total}</td><td className="px-3 py-2 text-xs text-success">{j.progress_done}</td><td className="px-3 py-2 text-xs text-destructive">{j.progress_failed}</td><td className="px-3 py-2 text-xs capitalize">{j.status}</td><td className="px-3 py-2 text-xs text-muted-foreground">{new Date(j.created_at).toLocaleDateString()}</td></tr>)}
        </tbody>
      </table>
