...
export function BulkImportTab({ config, onConfigChange, onJobCreated }: Props) {
  ...
  function extractUrls(raw: string): string[] {
    try { const json = JSON.parse(raw); if (Array.isArray(json)) return json.map(String).filter(u => u.startsWith("http")); } catch {}
  ...
  async function handleImport() {
    ...
    const res = await fetch("/api/importer/jobs", {
      ...
      body: JSON.stringify({ type: "bulk", urls, config }),
    });
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-border bg-background p-5 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">Paste multiple URLs (one per line), or upload a CSV/TXT/JSON file. Max 200 URLs per job.</p>
        ...
        <div className="border-2 border-dashed border-border rounded-lg p-4 text-center cursor-pointer hover:border-primary transition-colors"
          ...
          <p className="text-xs text-muted-foreground">CSV, TXT, JSON supported</p>
        ...
        <div className="relative">
          <textarea
            className="w-full rounded-lg border border-border bg-secondary/20 p-3 text-xs font-mono h-40 resize-none focus:outline-none focus:ring-2 focus:ring-primary"
            placeholder={"https://amazon.com/dp/B08N5WRWNW\nhttps://aliexpress.com/item/123456.html\nhttps://daraz.com.bd/products/..."}
            value={text}
            onChange={e => setText(e.target.value)}
          />
          {text && <button onClick={() => setText("")} className="absolute top-2 right-2 p-1 hover:bg-secondary rounded"><X size={12} /></button>}
        </div>

        {text && (
          <div className="flex items-center gap-2 text-sm">
            <span className="text-success font-medium">{urls.length} valid URLs detected</span>
            {urls.length > 200 && <span className="text-warning text-xs">(first 200 will be processed)</span>}
          </div>
        )}

        {error && <p className="text-sm text-destructive bg-destructive/10 rounded-lg px-3 py-2">{error}</p>}

        <Button onClick={handleImport} loading={loading} disabled={!urls.length}>
          Import {urls.length > 0 ? `${Math.min(urls.length, 200)} Products` : ""}
        </Button>
      </div>
