...
export function JobQueue({ onSelectJob, selectedJobId }: Props) {
  ...
  async function action(jobId: string, act: "cancel" | "pause" | "resume" | "process") {
    await fetch(`/api/importer/jobs/${jobId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: act }) });
    refresh();
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center mb-3">
        <p className="text-sm font-medium">{jobs.length} jobs</p>
        <button onClick={refresh} className="text-muted-foreground hover:text-foreground transition-colors"><RefreshCw size={14} /></button>
      </div>
      {jobs.map(job => {
        ...
        return (
          <div key={job.id}
            ...
            className={`rounded-lg border p-3 cursor-pointer transition-all ${isSelected ? "border-primary bg-primary/5" : "border-border hover:border-primary/40 bg-background"}`}>
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                {(job.status === "running" || job.status === "paused") && <button onClick={e => { e.stopPropagation(); action(job.id, "cancel"); }} className="p-1 hover:bg-secondary rounded text-destructive"><XCircle size={12} /></button>}
                {job.status === "completed" && job.progress_done < job.progress_total && <button onClick={e => { e.stopPropagation(); action(job.id, "process"); }} className="p-1 hover:bg-secondary rounded text-primary"><Play size={12} /></button>}
            ...
            {job.progress_total > 0 && (
              <div className="mt-2">
                <div className="flex justify-between text-xs text-muted-foreground mb-1">
                  <span>{job.progress_done} done · {job.progress_failed} failed</span>
                  <span>{pct}%</span>
                </div>
