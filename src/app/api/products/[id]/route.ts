...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  const { data, error } = await supabase.from("products").select("*, product_images(url, alt_text, position), categories(name, slug)").eq("id",id).single();
...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    const { id: _omit, images, categoryId, compareAtPrice, stockQuantity, brandId, ...rest } = parsed.data;
    ...
    await logAudit({ actorId:user.id, action: rest.status==="published"?"product.publish":"product.update", entityType:"product", entityId:id, metadata:{fields:Object.keys(update)} as Record<string,unknown>, ipAddress:ip });
