...
export async function GET(request: NextRequest) {
  ...
  let query = supabase
    .from("products")
    .select("id, title, slug, price, compare_at_price, currency, status, product_images(url, alt_text, position)", { count: "exact" })
    ...
    if (input.images.length) {
      await supabase.from("product_images").insert(
        input.images.map((img, i) => ({
          product_id: product.id, url: img.url, alt_text: img.altText ?? input.title, position: i,
        }))
      );
    }

    await logAudit({
      actorId: user.id, action: "product.create", entityType: "product", entityId: product.id,
      metadata: { title: input.title, status: input.status } as Record<string, unknown>, ipAddress: ip,
    });
