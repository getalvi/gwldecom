...
// GET /api/importer/jobs/:id — job status + items
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    const [{ data: job }, { data: items }] = await Promise.all([
      ...
      supabase.from("import_items").select("id, url, status, extraction_method, confidence, warnings, error_message, extracted, duration_ms, resulting_product_id").eq("job_id", id).order("created_at"),
    ]);
...
// PATCH /api/importer/jobs/:id — cancel, pause, resume, continue processing
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    if (body.action === "cancel") {
      await svc.from("import_jobs").update({ status: "cancelled", completed_at: new Date().toISOString() }).eq("id", id);
      await svc.from("import_items").update({ status: "skipped" }).eq("job_id", id).eq("status", "pending");
      return Response.json({ success: true });
    }

    if (body.action === "pause") {
      await svc.from("import_jobs").update({ status: "paused" }).eq("id", id);
      return Response.json({ success: true });
    }

    if (body.action === "resume" || body.action === "process") {
      await svc.from("import_jobs").update({ status: "running" }).eq("id", id);

      // Process next batch of pending items
      ...
      for (const item of pendingItems ?? []) {
        ...
        if (product) done++;
          ...
          failed++;
          await svc.from("import_items").update({ status: "failed", error_message: error }).eq("id", item.id);
        }
      }

      const { data: counts } = await supabase.from("import_items").select("status").eq("job_id", id);
      const totalDone = counts?.filter(c => c.status === "imported" || c.status === "preview" || c.status === "approved").length ?? 0;
      ...
      await svc.from("import_jobs").update({
        ...
        status: totalPending === 0 ? "completed" : "running",
        ...(totalPending === 0 ? { completed_at: new Date().toISOString() } : {}),
      }).eq("id", id);
