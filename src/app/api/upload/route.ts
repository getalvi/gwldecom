...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: uploadError } = await storage.storage.from("product-images").upload(path,processed.buffer,{contentType:processed.contentType,upsert:false,cacheControl:"31536000"});
    if (uploadError) return Response.json({ error: `Upload failed: ${uploadError.message}` }, { status: 500 });
    const { data: { publicUrl } } = storage.storage.from("product-images").getPublicUrl(path);
    return Response.json({url:publicUrl,width:processed.width,height:processed.height,bytes:processed.bytes}, { status: 201 });
