...
export default async function SearchPage({ searchParams }: Props) {
  ...
  function buildUrl(overrides: Record<string, string | undefined>) {
    ...
    const vals = { q, categoryId, sort, ...overrides };
    ...
    <main className="container py-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{count ?? 0} products</p>
        </div>
        <form method="GET" action="/search">
          {q && <input type="hidden" name="q" value={q} />}
          {categoryId && <input type="hidden" name="categoryId" value={categoryId} />}
          <select name="sort" defaultValue={sort ?? ""} className="h-9 rounded-md border border-border bg-background px-3 text-sm focus:outline-none">
        ...
        <aside className="md:col-span-1">
          <div className="rounded-xl border border-border bg-background p-4 sticky top-20">
            ...
            <div className="space-y-1">
              ...
              {(categories ?? []).map(cat => (
                <a key={cat.id} href={buildUrl({ categoryId: cat.id, page: undefined })}
        ...
        <div className="md:col-span-3">
          ...
          {(count ?? 0) > pageSize && (
            <div className="flex justify-center gap-2 mt-8">
              {page > 1 && (
                <a href={buildUrl({ page: String(page - 1) })} className="rounded-md border border-border px-4 py-2 text-sm hover:bg-secondary">← Prev</a>
              )}
              ...
              {page * pageSize < (count ?? 0) && (
                <a href={buildUrl({ page: String(page + 1) })} className="rounded-md border border-border px-4 py-2 text-sm hover:bg-secondary">Next →</a>
              )}
            </div>
          )}
        </div>
