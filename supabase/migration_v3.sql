-- =========================================================
-- MIGRATION V3 — AI Product Importer System
...
create table if not exists public.import_jobs (
  ...
  source_input jsonb not null,           -- raw input (urls, query, etc)
  config jsonb not null default '{}',    -- price rules, ai options, etc
...
-- Individual product extraction attempts within a job
create table if not exists public.import_items (
  ...
  extracted jsonb,          -- full structured extraction result
  extraction_method text,   -- 'jsonld','opengraph','css','ai','microdata'
...
create trigger import_items_updated before update on public.import_items
  for each row execute procedure public.set_updated_at();
